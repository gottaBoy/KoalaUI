...
define(function(require) {
	...
	Datagrid.prototype = {
		...
		renderColumns: function () {
			...
			$.each(this.columns, function (index, column) {
				...
				if(column.isHide) colHTML += ' style="display:none"';
		...
		updatePageDropdown: function (pages) {
			var pageHTML = '';
			...
			if(pages > 5){
				this.$pagedropdown.css({"overflow-x":"hidden","overflow-y":"auto","height":"120px"});
			}
		...
		render: function () {
			var self = this;
			var data = {};
			console.info(this.options.dataOptions);
			data.condition = JSON.stringify(this.options.dataOptions);
			$.post(this.options.url, data, function(result){
				...
				var itemdesc = (page.totalCount === 1) ? self.options.itemText : self.options.itemsText;
				var rowHTML = '';

				self.$footerchildren.css('visibility', function () {
					return (page.totalCount > 0) ? 'visible' : 'hidden';
				});
				
				self.$pageinput.val(page.pageNo);
				self.$pageslabel.text(page.totalPage);
				self.$countlabel.text(page.totalCount + ' ' + itemdesc);
				self.$startlabel.text(page.beginPos);
				self.$endlabel.text(page.endPos);
				self.updatePageDropdown(page.totalPage);
				self.updatePageButtons(page);
				$.each(result.datas,function(index,data){
					rowHTML += '<tr>';
					$.each(self.columns, function (index, column) {
						if(column.isHide){
							rowHTML += '<td style="display:none">' + data[column.property] + '</td>';
						}else{
							rowHTML += '<td>' + data[column.property] + '</td>';
						}
					});
					rowHTML += '</tr>';
				});
		...
		filterChanged: function (e, filter) {
			this.options.dataOptions.filter = filter.value;
			this.options.dataOptions.pageIndex = 1;
			this.render();
		},
	...
	$.fn.datagrid.defaults = {
		dataOptions: { pageIndex: 1, pageSize: 10 },
